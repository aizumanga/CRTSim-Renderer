...
fn border_load(xy: vec2<i32>) -> vec3<f32> {
    ...
    return textureLoad(source,xy,0).rgb;
}
...
fn crt(uv: vec2<f32>, frame: bool) -> vec3<f32> {
    ...
    var grid=textureSampleLevel(mask_tex,linear_repeat,scaled*density,0.).rgb;
    ...
    return mix(vec3(luma(emissive)),emissive,p.surface.z);
}
fn shade(v: Surface, frame: bool) -> vec4<f32> {
    let n=normalize(v.normal); let cam=normalize(v.camera); let light=normalize(v.light);
    let diffuse=max(dot(n,light),0.);
    let halfvec=normalize(light+cam);
    ...
    if frame {
        ...
        color=p.frame.rgb*(diffuse+hemi)*p.lighting.x + vec3(0.25)*spec*p.lighting.y
...
@fragment fn present(q: Quad) -> @location(0) vec4<f32> {
    ...
    return vec4(base+colored*p.bloom.x,1.);
}
